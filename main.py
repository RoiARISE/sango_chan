    ...
    if note.get('mentions'): # メンションされるかつ同じワードが含まれているときに返信するやつ
        if MY_ID in note['mentions']:
            ...
            if "回線速度計測" in note['text']: # 現在接続しているインターネットの回線速度を計測する機能
                if note["user"]["id"] == "abcdefg1234567": # プロフィールの「RAW」にある「ID」で判断。これを設定すると機能を自分専用にできる。多分もっといい方法があるはず
                    import speedtest 
                    ...
                    return
                if not note["user"]["id"] == "abcdefg1234567": # 自分以外使えないようにするプログラム。これがないと多分悪用されかねない
